...
class MarkerDocking(Node):
    def __init__(self):
        ...
        self.cmd_vel_pub = self.create_publisher(Twist, "cmd_vel_aruco", 1)
    ...
    def aruco_callback(self, msg):
        thres_dist = 0.3
        for info in msg.markers:
            if info.marker_id == 3 or info.marker_id == 4:
                T_cm = transformations.quaternion_matrix([info.transform.rotation.x,
                                                       info.transform.rotation.y,
                                                       info.transform.rotation.z,
                                                       info.transform.rotation.w])
                t = np.array([info.transform.translation.x,
    ...
    def align_run(self):
        if self.dist_virtual == 1e3 and self.step != CalibMode.READY2ALIGN: return
        const_vel = 0.04
        const_ang = 0.05
        if self.docking == DockingMode.STATUS_DOCKING:
            if self.dist_virtual <= 0.05: # 2. drived in sufficient distance, stop(will substitute to work function), when finished entering to unloading mode
                return
                self.linear_x = 0.0
                self.angular_z = 0.0
