...
class MarkerDocking(Node):
    ...
    def aruco_callback(self, msg):
        ...
        T_cm = np.vstack([np.hstack([R, t]), np.array([0, 0, 0, 1])])
        TT = np.array([0, 0, 0.2, 1])
        T_virtual = np.matmul(T_cm, TT)

        euler_rad = -cv2.decomposeProjectionMatrix(T_cm)[6] * math.pi/180
        self.distance = math.sqrt(T_virtual[0] ** 2 + T_virtual[2] ** 2)
        self.angular_euler = -euler_rad[1][0] # ??
        self.angular = math.sqrt(T_virtual[0] ** 2 + T_virtual[2] ** 2)
    ...
    def run(self):
        if self.docking == DockingMode.STATUS_DOCKING:
            if self.distance <= 0.05: # 2. drived in sufficient distance, stop(will substitute to work function), when finished entering to unloading mode
                self.linear_x = 0
                self.angular_z = 0
            ...
            self.cmd_vel_pub.publish(Twist(linear=Vector3(self.linear_x, 0.0, 0.0), angular=Vector3(0.0, 0.0, -self.angular_z)))
