...
class BasicNavigator(Node):
    def __init__(self, namespace):
        super().__init__(node_name=namespace + 'basic_navigator')
        ...
        self.initial_pose_pub = self.create_publisher(PoseWithCovarianceStamped,
                                                      ...
                                                      10)
        self.change_maps_srv = self.create_client(LoadMap, self.namespace[:-1] + '/map_server/load_map')
        self.clear_costmap_global_srv = self.create_client(
            ClearEntireCostmap, self.namespace[:-1] + '/global_costmap/clear_entirely_global_costmap')
        self.clear_costmap_local_srv = self.create_client(
            ClearEntireCostmap, self.namespace[:-1] + '/local_costmap/clear_entirely_local_costmap')
        self.get_costmap_global_srv = self.create_client(GetCostmap, self.namespace[:-1] + '/global_costmap/get_costmap')
        self.get_costmap_local_srv = self.create_client(GetCostmap, self.namespace[:-1] + '/local_costmap/get_costmap')
    ...
    def waitUntilNav2Active(self):
        ...
        self._waitForNodeToActivate(self.namespace + 'amcl')
        self._waitForInitialPose()
        self._waitForNodeToActivate(self.namespace + 'bt_navigator')
        self.info(self.namespace[:-1] + ' Nav2 is ready for use!')
        return
    ...
    def _waitForNodeToActivate(self, node_name):
        ...
        while not state_client.wait_for_service(timeout_sec=0.1):
