...
var othermarker1 = new ROS2D.NavigationArrow({
  size : 0.5,
  strokeSize : 0.01,
  pulse: true,
...
var othermarker2 = new ROS2D.NavigationArrow({
  size : 0.5,
  strokeSize : 0.01,
  pulse: true,
  fillColor: createjs.Graphics.getRGB(0,0,255, 0.9),
...
const createmark = function(topic,frame_id,marker)
{
  return topic.subscribe(function(pose){
    console.log(pose);
    if(frame_id == "markermap_1" && !t1 && pose.transforms[0].child_frame_id == "markermap_1")
    {
    let odomPose = pose.transforms[0].transform.translation
    
    let quaZ=pose.transforms[0].transform.rotation.z
    ...
    // degree
    robotMarker.rotation = degreeZ;
    gridClient.rootObject.addChild(marker);
    t1 = 1;
    }
    if(frame_id == "markermap_2" && !t2 && pose.transforms[0].child_frame_id == "markermap_2")
    {
      let odomPose = pose.transforms[0].transform.translation
...
createmark(new ROSLIB.Topic({
  ros : ros,
  name : '/tf',
  messageType: 'tf2_msgs/TFMessage'
}),"markermap_1",othermarker1);

createmark(new ROSLIB.Topic({
  ros : ros,
  name : '/tf',
  messageType: 'tf2_msgs/TFMessage'
}),"markermap_2",othermarker2);
